...
Опциональный массив `checkpoints` хранит точные контрольные позиции отдельных
кадров:
...
```json
{
  ...
  "layer_id": "surface",
  "position": {"x": 1234.5, "y": 678.9, "tolerance_px": 35}
}
```

Checkpoint относится к ближайшему записанному кадру. Его можно добавить через
`annotate-scenario`; абсолютные пути, полный экран и UID в manifest не попадают.
Старые scenario v1 без `checkpoints` остаются совместимыми.
