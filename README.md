...
Если включены блоки `poi` и `navigation`, отдельное окно показывает официальные POI
текущего слоя, игрока, закреплённую цель, прямую линию и north-up стрелку направления.
Первоначально выбирается ближайший несобранный сундук, но при движении цель сама не
переключается. Управление работает только при активном окне Navigator:
...
Когда фокус остаётся на Genshin, те же действия доступны глобально при включённом
`Num Lock`: `Num4` / `Num6` — предыдущая / следующая цель, `Num2` — пропустить,
`Num5` — отметить собранной, `Num8` — отменить. Navigator только читает состояние
этих клавиш и не отправляет ввод в игру.
...
и сереют. Navigator никогда не выбирает POI другого региона, этажа или системы
координат.
